#!/usr/bin/env bash
# 0_db2-install.sh — install Db2, create the db2inst1 instance + SAMPLE database,
...
#   sudo ./scripts/0_db2-install.sh /path/to/server_dec     # dir containing db2_install
#
...
#   - Run 0_opensearch-install.sh first (OpenSearch registration is catalog metadata,
#     so OpenSearch need only be installed here, not running).
set -euo pipefail

MEDIA="${1:?Usage: sudo ./scripts/0_db2-install.sh /path/to/server_dec}"
